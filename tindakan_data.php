...
$ambil = $koneksidb->query("SELECT * FROM tindakan");
...
<div class="table-responsive">
	...
	$ambil = $koneksidb->query("SELECT * FROM tindakan ORDER BY kd_tindakan ASC LIMIT $hal, $row");
  $pecah = $ambil->fetch_assoc();
	//$myQry = mysql_query($ambil, $koneksidb)  or die ("Query salah : ".mysql_error());
	$nomor = 0; 
	while ($myData = mysqli_fetch_array($ambil)) {
		$nomor++;
