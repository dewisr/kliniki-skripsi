...
if(isset($_POST['btnLogin'])){
	...
	if (count($pesanError)>=1 ){
		...
		$ambil = $koneksidb->query("SELECT * FROM petugas WHERE username='".$txtUser."' AND password='".md5($txtPassword)."' AND level='$cmbLevel'");
		...
		if(mysqli_num_rows($ambil) >=1) {
			$_SESSION['SES_LOGIN'] = $pecah['kd_petugas']; 
			$_SESSION['SES_USER'] = $pecah['username']; 
